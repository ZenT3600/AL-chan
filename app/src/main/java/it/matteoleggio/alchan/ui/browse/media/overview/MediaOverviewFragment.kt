...
import it.matteoleggio.alchan.helper.pojo.*
...
class MediaOverviewFragment : BaseFragment() {
    ...
    private fun assignCharactersAdapter(): OverviewCharactersRvAdapter {
        ...
        return OverviewCharactersRvAdapter(requireActivity(), viewModel.charactersList, width, object : OverviewCharactersRvAdapter.OverviewCharactersListener {
    ...
    private fun assignTagsAdapter(): OverviewTagsRvAdapter {
        return OverviewTagsRvAdapter(requireActivity(), if (!viewModel.showSpoiler) viewModel.tagsList.filter { it.isMediaSpoiler == false && it.isGeneralSpoiler == false } else viewModel.tagsList, object : OverviewTagsRvAdapter.OverviewTagsListener {
    ...
    private fun assignRelationsAdapter(): OverviewRelationsRvAdapter {
        ...
        return OverviewRelationsRvAdapter(requireActivity(), viewModel.relationsList, width, object : OverviewRelationsRvAdapter.OverviewRelationsListener {
    ...
    private fun assignTrailersAdapter(): OverviewTrailersRvAdapter {
        ...
        activity?.windowManager?.defaultDisplay?.getMetrics(metrics)
        val width = (metrics.widthPixels / 1.3).toInt()
        return OverviewTrailersRvAdapter(requireActivity(), viewModel.trailersList, width, object : OverviewTrailersRvAdapter.OverviewTrailersListener {
                ...
                DialogUtility.showOptionDialog(
                    ...
                    R.string.watch,
                    {
                        CustomTabsIntent.Builder()
                            .build()
                            .launchUrl(requireActivity(), Uri.parse(site))
                    },
                    R.string.cancel,
                    { }
                )
    ...
    private fun assignLinksAdapter(): OverviewLinksRvAdapter {
        return OverviewLinksRvAdapter(requireActivity(), viewModel.linksList, object : OverviewLinksRvAdapter.OverviewLinksListener {
