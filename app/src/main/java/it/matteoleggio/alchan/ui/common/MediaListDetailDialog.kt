...
class MediaListDetailDialog : DialogFragment() {
    ...
    override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
        ...
        val dialogView = requireActivity().layoutInflater.inflate(R.layout.dialog_media_list_detail, mediaListDetailLayout, false)
        ...
        if (scoreFormat == ScoreFormat.POINT_3) {
            GlideApp.with(requireActivity()).load(AndroidUtility.getSmileyFromScore(mediaList.score)).into(dialogView.mediaListScoreIcon)
            ...
            GlideApp.with(requireActivity()).load(R.drawable.ic_star_filled).into(dialogView.mediaListScoreIcon)
            dialogView.mediaListScoreIcon.imageTintList = ColorStateList.valueOf(ContextCompat.getColor(requireActivity(), R.color.yellowStar))
