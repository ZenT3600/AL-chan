...
import it.matteoleggio.alchan.ui.search.adapter.*
...
class SearchListFragment : Fragment() {
    ...
    private fun assignAdapter() {
        adapter = when (viewModel.searchPage) {
            BrowsePage.ANIME -> SearchAnimeRvAdapter(requireActivity(), viewModel.searchResultList, searchListenerObject(BrowsePage.ANIME))
            BrowsePage.MANGA -> SearchMangaRvAdapter(requireActivity(), viewModel.searchResultList, searchListenerObject(BrowsePage.MANGA))
            BrowsePage.CHARACTER -> SearchCharactersRvAdapter(requireActivity(), viewModel.searchResultList, false, searchListenerObject(BrowsePage.CHARACTER))
            BrowsePage.STAFF -> SearchStaffsRvAdapter(requireActivity(), viewModel.searchResultList, false, searchListenerObject(BrowsePage.STAFF))
            BrowsePage.STUDIO -> SearchStudioRvAdapter(requireActivity(), viewModel.searchResultList, false, searchListenerObject(BrowsePage.STUDIO))
            BrowsePage.USER -> SearchUsersRvAdapter(requireActivity(), viewModel.searchResultList, searchListenerObject(BrowsePage.USER))
            else -> SearchAnimeRvAdapter(requireActivity(), viewModel.searchResultList, searchListenerObject(BrowsePage.ANIME))
        }
